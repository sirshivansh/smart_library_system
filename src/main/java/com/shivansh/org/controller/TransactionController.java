...
/**
 * Controller class for Transaction-related operations.
 * Handles book borrowing, returning, transaction history,
 * overdue reporting, and library statistics aggregation.
 * 
 * @author Shivansh
 * @version 1.0
 */
public class TransactionController {
    ...
    public boolean borrowBook(int memberId, int bookId) throws TransactionException {
        ...
        if (!result) {
            throw new TransactionException("Failed to issue book. Check availability and borrowing limits.");
        }
        return true;
    }
    ...
    public double returnBook(int memberId, int bookId) throws TransactionException {
        ...
        Transaction activeTx = memberTxs.stream()
        ...
        if (!result) {
            throw new TransactionException("Return failed. Ensure you have an active borrow for this book.");
        }
    ...
    /**
     * Retrieves all currently overdue transactions (ISSUED status with due date in the past).
     * This is used by the admin Overdue Report feature.
     * 
    ...
    public List<Transaction> getOverdueTransactions() {
        return transactionService.getAllTransactions().stream()
                ...
                .filter(t -> t.getDueDate() != null && t.getDueDate().toLocalDate().isBefore(LocalDate.now()))
                .collect(Collectors.toList());
    }

    /**
     * Computes library-wide statistics for the admin dashboard.
     * Returns an array: [totalTitles, totalCopies, availableCopies, totalMembers, activeBorrows, overdueCount, totalFines]
     * 
    ...
    public double[] getLibraryStatistics() {
        ...
        double activeBorrows = transactions.stream().filter(t -> "ISSUED".equals(t.getStatus())).count();

        double overdueCount = transactions.stream()
                .filter(t -> "ISSUED".equals(t.getStatus()))
                .filter(t -> t.getDueDate() != null && t.getDueDate().toLocalDate().isBefore(LocalDate.now()))
                .count();

        double totalFines = transactions.stream()
                .mapToDouble(Transaction::getCalculatedFine)
                .sum();

        return new double[]{totalTitles, totalCopies, availableCopies, totalMembers, activeBorrows, overdueCount, totalFines};
    }
}
