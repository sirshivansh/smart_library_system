...
public class TransactionServiceImpl implements TransactionService {
  ...
  public boolean borrowBook(int memberId, int bookId) {
    ...
    if (activeTx != null) {
      System.out.println(
          "You have already issued this book and not returned it yet (Transaction ID: "
              + activeTx.getTransactionId()
              + ").");
      return false;
    }
    ...
    long activeCount = memberTxs.stream().filter(t -> "ISSUED".equals(t.getStatus())).count();

    int limit = 3; // Regular limit
    String type =
        member.getMembershipType() != null ? member.getMembershipType().toUpperCase() : "REGULAR";
    ...
    if (activeCount >= limit) {
      System.out.println(
          "Borrowing limit reached! As a "
              + type
              + " member, you can borrow a maximum of "
              + limit
              + " books at a time. Currently borrowed: "
              + activeCount);
      return false;
    }
  ...
  public boolean returnBook(int memberId, int bookId) {
    ...
    if (success) {
      ...
      if (fine > 0) {
        System.out.printf(
            "NOTICE: This book was returned late! An overdue fine of %.2f rupees has been recorded.%n",
            fine);
      }
    }
    return success;
