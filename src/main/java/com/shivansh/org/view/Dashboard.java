 ...
 * 
 * Implements the View layer of the MVC architecture:
 *   - Model:      DTOs (Book, Member, Transaction) + DAO layer
 *   - View:       This class (Dashboard) — Console UI rendering
 *   - Controller: BookController, MemberController, TransactionController
 * 
 * Features a premium CLI interface with ANSI colors, Unicode box-drawing,
 * ASCII art branding, real-time library statistics, and overdue reports.
 * 
 * @author Shivansh
 * @version 2.0
 */
public class Dashboard {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (mainOption) {
                case 1: handleAdminLogin(br); break;
    ...
    private static void printBanner() {
        ...
        System.out.println(CYAN_BOLD + "  ██████╔╝╚██████╔╝╚██████╔╝██║  ██╗    ██║  ██║██║  ██║ ╚████╔╝ ███████╗██║ ╚████║" + RESET);
        System.out.println(CYAN_BOLD + "  ╚══════╝  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝    ╚═╝  ╚═╝╚═╝  ╚═╝  ╚═══╝  ╚══════╝╚═╝  ╚═══╝" + RESET);
        System.out.println(WHITE_BOLD + "              S M A R T   L I B R A R Y   M A N A G E M E N T   S Y S T E M" + RESET);
        System.out.println(DIM + "                          MVC Architecture  •  MySQL  •  Java 11" + RESET);
    ...
    private static void showAdminDashboard(BufferedReader br) {
        ...
        do {
            ...
            System.out.println("  " + YELLOW_BOLD + " 1 " + RESET + " │ " + GREEN + "Add" + RESET + " New Book to Catalog");
            System.out.println("  " + YELLOW_BOLD + " 2 " + RESET + " │ " + BLUE + "Update" + RESET + " Book Catalog Entry");
            System.out.println("  " + YELLOW_BOLD + " 3 " + RESET + " │ " + RED + "Delete" + RESET + " Book from System");
            System.out.println("  " + YELLOW_BOLD + " 4 " + RESET + " │ View Full Book Catalog");
            System.out.println("  " + YELLOW_BOLD + " 5 " + RESET + " │ Search Catalog (Title/Author/Genre)");
            printDivider();
            ...
            System.out.println("  " + YELLOW_BOLD + " 8 " + RESET + " │ " + RED + "Delete" + RESET + " Member from System");
            printDivider();
            System.out.println("  " + YELLOW_BOLD + " 9 " + RESET + " │ View All Transaction Logs");
            System.out.println("  " + YELLOW_BOLD + "10 " + RESET + " │ " + RED_BOLD + "★" + RESET + " Generate Overdue Report");
            ...
            try {
                switch (choice) {
                    case 1: adminAddBook(br); break;
                    case 2: adminUpdateBook(br); break;
                    case 3: adminDeleteBook(br); break;
                    case 4:
    ...
    private static void adminUpdateBook(BufferedReader br) throws BookNotFoundException, ValidationException {
    ...
    private static void adminDeleteBook(BufferedReader br) throws BookNotFoundException {
        ...
        String confirm = readString(br, "  " + RED + "Confirm permanent deletion? (yes/no): " + RESET, true);
    ...
    private static void adminDeleteMember(BufferedReader br) throws MemberNotFoundException {
        ...
        if ("yes".equalsIgnoreCase(confirm.trim())) {
            if (memberController.deleteMember(id)) {
                printSuccess("Member ID " + id + " (" + m.getFirstName() + " " + m.getLastName() + ") permanently removed.");
            } else {
                printError("Deletion failed.");
            }
        } else {
    ...
    private static void adminOverdueReport() {
        ...
        if (overdue.isEmpty()) {
            ...
            double totalFines = overdue.stream()
                    .mapToDouble(Transaction::getCalculatedFine)
                    .sum();
            System.out.println("  " + RED_BOLD + "  Total Overdue Fines: ₹" + String.format("%.2f", totalFines) + RESET);
        }
    }
    ...
    private static void showMemberDashboard(BufferedReader br, Member member) {
        ...
        do {
            printBanner();
            printBoxHeader("MEMBER DASHBOARD: " + member.getFirstName().toUpperCase() + " " + member.getLastName().toUpperCase());
            System.out.println();
            printSectionTitle("OPERATIONS");
            ...
            try {
                switch (choice) {
                    ...
                    case 5:
                        ...
                        printTransactionTable(transactionController.getMemberTransactions(member.getMemberId()));
                        break;
                    case 6:
    ...
    private static void printBookTable(List<Book> books) {
        ...
        System.out.printf(border + "  ║ %-6s │ %-30s │ %-24s │ %-14s │ %-14s │ %-8s │ %-8s ║%n" + RESET,
                "ID", "Title", "Author", "ISBN", "Genre", "Total", "Avail");
        System.out.println(border + "  ╠════════╪════════════════════════════════╪══════════════════════════╪════════════════╪════════════════╪══════════╪══════════╣" + RESET);
        for (Book b : books) {
            String availColor = b.getAvailableCopies() > 0 ? GREEN : RED;
            System.out.printf("  ║ %-6d │ %-30s │ %-24s │ %-14s │ %-14s │ %-8d │ " + availColor + "%-8d" + RESET + " ║%n",
                    b.getBookId(), trunc(b.getTitle(), 30), trunc(b.getAuthor(), 24),
                    trunc(b.getIsbn() != null ? b.getIsbn() : "—", 14),
                    trunc(b.getGenre(), 14), b.getTotalCopies(), b.getAvailableCopies());
        }
        System.out.println(border + "  ╚════════╧════════════════════════════════╧══════════════════════════╧════════════════╧════════════════╧══════════╧══════════╝" + RESET);
    ...
    private static void printMemberTable(List<Member> members) {
        ...
        System.out.printf(b + "  ║ %-6s │ %-30s │ %-34s │ %-12s │ %-15s ║%n" + RESET,
                "ID", "Name", "Email", "Phone", "Type");
        System.out.println(b + "  ╠════════╪════════════════════════════════╪════════════════════════════════════╪══════════════╪═════════════════╣" + RESET);
        for (Member m : members) {
            System.out.printf("  ║ %-6d │ %-30s │ %-34s │ %-12s │ %-15s ║%n",
                    m.getMemberId(), trunc(m.getFirstName() + " " + m.getLastName(), 30),
                    trunc(m.getEmail(), 34),
    ...
    private static void printTransactionTable(List<Transaction> txs) {
        ...
        String b = CYAN_BOLD;
        System.out.println(b + "  ╔═══════╤═════════════════════════╤═════════════════════════╤════════════╤════════════╤════════════╤══════════╤══════════╗");
        System.out.printf(b + "  ║ %-5s │ %-23s │ %-23s │ %-10s │ %-10s │ %-10s │ %-8s │ %-8s ║%n" + RESET,
                "TxID", "Member", "Book", "Issued", "Due", "Returned", "Status", "Fine");
        System.out.println(b + "  ╠═══════╪═════════════════════════╪═════════════════════════╪════════════╪════════════╪════════════╪══════════╪══════════╣" + RESET);
        for (Transaction t : txs) {
            ...
            System.out.printf("  ║ %-5d │ %-23s │ %-23s │ %-10s │ %-10s │ %-10s │ " + statusColor + "%-8s" + RESET + " │ %-8s ║%n",
                    t.getTransactionId(),
                    trunc(t.getMemberName() != null ? t.getMemberName() : String.valueOf(t.getMemberId()), 23),
                    trunc(t.getBookTitle() != null ? t.getBookTitle() : String.valueOf(t.getBookId()), 23),
                    t.getIssueDate(), t.getDueDate(), retDate, t.getStatus(), fineStr);
        }
        System.out.println(b + "  ╚═══════╧═════════════════════════╧═════════════════════════╧════════════╧════════════╧════════════╧══════════╧══════════╝" + RESET);
    ...
    private static void printMemberCard(Member m) {
        ...
        System.out.printf("  " + MAGENTA_BOLD + "│" + RESET + "  Member ID:       " + BOLD + "%-30d" + MAGENTA_BOLD + "│%n" + RESET, m.getMemberId());
        System.out.printf("  " + MAGENTA_BOLD + "│" + RESET + "  Full Name:       %-30s" + MAGENTA_BOLD + "│%n" + RESET, m.getFirstName() + " " + m.getLastName());
        System.out.printf("  " + MAGENTA_BOLD + "│" + RESET + "  Email:           %-30s" + MAGENTA_BOLD + "│%n" + RESET, m.getEmail());
        System.out.printf("  " + MAGENTA_BOLD + "│" + RESET + "  Phone:           %-30s" + MAGENTA_BOLD + "│%n" + RESET, m.getPhone() != null ? m.getPhone() : "—");
        System.out.printf("  " + MAGENTA_BOLD + "│" + RESET + "  Membership:      %-30s" + MAGENTA_BOLD + "│%n" + RESET, m.getMembershipType());
        System.out.println("  " + MAGENTA_BOLD + "└──────────────────────────────────────────────────┘" + RESET);
    }
    ...
    private static void printAdminStats() {
        ...
        System.out.printf("  " + YELLOW_BOLD + "│  " + CYAN_BOLD + "%-34s" + YELLOW_BOLD + " │  " + CYAN_BOLD + "%-35s" + YELLOW_BOLD + "│%n" + RESET, "CATALOG SUMMARY", "CIRCULATION METRICS");
        System.out.println("  " + YELLOW_BOLD + "├─────────────────────────────────────┼──────────────────────────────────────┤" + RESET);
        System.out.printf("  " + YELLOW_BOLD + "│  " + RESET + "Book Titles:       " + BOLD + "%-16d" + YELLOW_BOLD + " │  " + RESET + "Active Borrows:    " + BOLD + "%-16d" + YELLOW_BOLD + " │%n" + RESET, totalTitles, activeBorrows);
        System.out.printf("  " + YELLOW_BOLD + "│  " + RESET + "Physical Copies:   " + BOLD + "%-16d" + YELLOW_BOLD + " │  " + RESET + "Overdue Items:     " + RED_BOLD + "%-16d" + YELLOW_BOLD + " │%n" + RESET, totalCopies, overdueCount);
        System.out.printf("  " + YELLOW_BOLD + "│  " + RESET + "Available Copies:  " + GREEN_BOLD + "%-16d" + YELLOW_BOLD + " │  " + RESET + "Pending Fines:     " + RED_BOLD + "₹%-15.2f" + YELLOW_BOLD + " │%n" + RESET, availCopies, totalFines);
        System.out.printf("  " + YELLOW_BOLD + "│  " + RESET + "Registered Members:" + BLUE_BOLD + "%-16d" + YELLOW_BOLD + " │  " + RESET + "                                     " + YELLOW_BOLD + "│%n" + RESET, totalMembers);
        System.out.println("  " + YELLOW_BOLD + "└─────────────────────────────────────┴──────────────────────────────────────┘" + RESET);
    }
